...
public class SimpleManipulations {
    ...
    public static PixelPicture border(
        ...
        int h = pic.getHeight() + borderWidth * 2;
        int w = pic.getWidth() + borderWidth * 2;
        Pixel[][] bmp = new Pixel[h][w];
        Pixel[][] src = pic.getBitmap();

        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                if (col < (borderWidth) || col > borderWidth + pic.getWidth() - 1
                        || row < (borderWidth) || row > borderWidth + pic.getHeight() - 1) {
                    bmp[row][col] = borderColor;
                } else {
                    bmp[row][col] = src[row - borderWidth][col - borderWidth];
                }
            }
        }
    ...
    public static PixelPicture invertColors(PixelPicture pic) {
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                int r = Math.abs(255 - src[row][col].getRed());
                int g = Math.abs(255 - src[row][col].getGreen());
                int b = Math.abs(255 - src[row][col].getBlue());
    ...
    public static PixelPicture scaleColors(
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                int r = (int) Math.round(src[row][col].getRed() * rfactor);
                int g = (int) Math.round(src[row][col].getGreen() * gfactor);
                int b = (int) Math.round(src[row][col].getBlue() * bfactor);
                bmp[row][col] = new Pixel(r, g, b);
            }
        }
    ...
    public static PixelPicture alphaBlend(
        ...
        Pixel[][] bmp = new Pixel[h][w];
        Pixel[][] picpix = pic.getBitmap();
        Pixel[][] fpix = f.getBitmap();
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                int r = weightedAverage(
                        alpha, picpix[row][col].getRed(),
                        fpix[row][col].getRed()
                );
                int g = weightedAverage(
                        alpha, picpix[row][col].getGreen(),
                        fpix[row][col].getGreen()
                );
                int b = weightedAverage(
                        alpha, picpix[row][col].getBlue(),
                        fpix[row][col].getBlue()
                );
                bmp[row][col] = new Pixel(r, g, b);
            }
        }
