...
public class AdvancedManipulations {
    ...
    public static PixelPicture adjustContrast(
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ag += (src[row][col].getRed()
                        + src[row][col].getGreen()
                        + src[row][col].getBlue());
            }
        }

        double agC = ag / (3.0 * w * h);
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                int r = (int) Math.round(
                        ((src[row][col].getRed() - agC)
                                * multiplier) + agC
                );
                int g = (int) Math.round(
                        ((src[row][col].getGreen() - agC)
                                * multiplier) + agC
                );
                int b = (int) Math.round(
                        ((src[row][col].getBlue() - agC)
                                * multiplier) + agC
                );
    ...
    public static PixelPicture reducePalette(PixelPicture pic, int numColors) {
        ...
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                Pixel p = bmp[row][col];
                int frequency = 1;
                if (picMap.contains(p)) {
                    frequency = picMap.getValue(p) + 1;
                }
                picMap.put(p, frequency);
            }
        }
        Pixel[] picArray = picMap.getSortedPixels();
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                Pixel p = bmp[row][col];
                Pixel closest = picArray[0];
                for (int i = 1; i < numColors; i++) {
                    if (p.distance(picArray[i]) < p.distance(closest)) {
                        closest = picArray[i];
                    }
                }
                bmp[row][col] = closest;
    ...
    public static PixelPicture blur (PixelPicture pic, int radius) {
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                bmp[row][col] = pixBoxAverage(src, row, col, radius);
            }
        }
    ...
    public static Pixel pixBoxAverage(Pixel[][] bmp, int tempR, int tempC, int radius) {
        int r = 0;
        int g = 0;
        int b = 0;

        int realcleft = tempC - radius;
        int realrleft = tempR - radius;
        int realcright = tempC + radius + 1;
        int realrright = tempR + radius + 1;
        if (realcleft < 0) {
            realcleft = 0;
        }
        if (realrleft < 0) {
        ...
        if (realcright > bmp[0].length) {
            realcright = bmp[0].length;
        }
        if (realrright > bmp.length) {
            realrright = bmp.length;
        }
        for (int col = realcleft; col < realcright; col++) {
            for (int row = realrleft; row < realrright; row++) {
                r += bmp[row][col].getRed();
                g += bmp[row][col].getGreen();
                b += bmp[row][col].getBlue();
            }
        }
        System.out.println(r);
        double num = ((Math.abs(realrleft - realrright))
                * (Math.abs(realcleft - realcright)));
        r = (int) Math.round(r / num);
        g = (int) Math.round(g / num);
        b = (int) Math.round(b / num);
